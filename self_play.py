...
class SelfPlay:
    def __init__(self, dim, count, AI1_params, AI2_params,
                 outfile=None, verbose=False, eval_model=None, loss=mse_loss):
        self.dim = dim
        self.count = count
    ...
    def play(self, AI):
        ...
        if self.eval_model:
            pred = float(self.eval_model(self.caro_board.get_board(), self.caro_board.get_dim()))
            if self.loss:
        ...
        self.data_points.append(create_data_point(AI, self.caro_board))
        if self.outfile:
            save_raw_data(self.outfile, AI, self.caro_board)
        self.data_count += 1
