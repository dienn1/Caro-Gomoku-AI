...
PYBIND11_MODULE(MCTS_pybind, m)
{
	py::class_<TreeNode>(m, "TreeNode")
		...
		.def_readwrite("win", &TreeNode::win)
		...
		.def("winrate", &TreeNode::winrate)
		.def("uct", &TreeNode::uct)
		.def("get_player", &TreeNode::get_player)
		.def("__str__", &TreeNode::to_string);

	py::class_<MCTS_AI>(m, "MCTS_AI")
		.def(py::init<int, int, int, Caro const&, int, std::function<double(std::array<std::array<int, 30>, 30>, int)>>(),
		...
		.def("predicted_winrate", &MCTS_AI::predicted_winrate)
